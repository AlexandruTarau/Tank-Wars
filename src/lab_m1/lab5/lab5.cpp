...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(1, 2.5f, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_W)) {
            ...
            camera->TranslateForward(distance * deltaTime);
        }

        if (window->KeyHold(GLFW_KEY_A)) {
            // TODO(student): Translate the camera to the left
            camera->TranslateRight(-distance * deltaTime);
        }
        ...
        if (window->KeyHold(GLFW_KEY_Q)) {
            ...
            camera->TranslateUpward(-distance * deltaTime);
        }
        ...
        if (window->KeyHold(GLFW_KEY_L)) {
            ortho_left += 10 * deltaTime;
            ortho_up += 10 * deltaTime;
            projectionMatrix = glm::ortho(-ortho_left, ortho_left, -ortho_up, ortho_up, 0.01f, 200.0f);
        }
